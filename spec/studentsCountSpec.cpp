...
namespace spec
{
	...
	public:
		...
		TEST_METHOD(inValidInputCheck)
		{
			...
			Assert::IsNull(studentsCount_way2(NULL, 1, 1, &lessCount, &moreCount), L"Array NULL check failed.", LINE_INFO());
			int Arr1[1] = { 1 };
			Assert::IsNull(studentsCount_way2(Arr1, -2, 1, &lessCount, &moreCount), L"Length less than zero check failed.", LINE_INFO());
		}
		
		TEST_METHOD(singleElement)
		{
			int Arr1[1] = { 1 }, lessCount, moreCount;
			studentsCount_way2(Arr1, 1, 1, &lessCount, &moreCount);
			...
			int Arr2[1] = { 1 };
			studentsCount_way2(Arr2, 1, 12, &lessCount, &moreCount);
			Assert::AreEqual(1, lessCount, L"Single element lessCount 1 case failed.", LINE_INFO());
			Assert::AreEqual(0, moreCount, L"Single element moreCount 0 case failed.", LINE_INFO());
			int Arr3[1] = { 111 };
			studentsCount_way2(Arr3, 1, 12, &lessCount, &moreCount);
			Assert::AreEqual(0, lessCount, L"Single element lessCount 0 case failed.", LINE_INFO());
		...
		TEST_METHOD(sameElements)
		{
			...
			studentsCount_way2(Arr, 4, 12, &lessCount, &moreCount);
		...
		TEST_METHOD(differentElements)
		{
			...
			studentsCount_way2(Arr, 5, 30, &lessCount, &moreCount);
			...
			studentsCount_way2(Arr1, 5, 30, &lessCount, &moreCount);
